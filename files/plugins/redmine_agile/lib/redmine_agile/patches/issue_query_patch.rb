    ...
    module IssueQueryPatch
      def self.included(base)
        ...
        base.class_eval do
          ...
          alias_method :joins_for_order_statement_without_agile, :joins_for_order_statement
          alias_method :joins_for_order_statement, :joins_for_order_statement_with_agile
        end
      end
      ...
      module InstanceMethods
        ...
        def joins_for_order_statement_with_agile(order_options)
          joins = joins_for_order_statement_without_agile(order_options)
          agile_joins = [joins].flatten
          if order_options && order_options.include?('agile_sprint')
            agile_joins << "LEFT OUTER JOIN #{AgileData.table_name} ON #{Issue.table_name}.id = #{AgileData.table_name}.issue_id"
          end
          agile_joins.any? ? agile_joins.join(' ') : nil
        end
        ...
        def available_columns_with_agile
          if @available_columns.blank?
            ...
            @available_columns << QueryColumn.new(:agile_sprint, caption: :field_agile_sprint, sortable: "#{AgileData.table_name}.agile_sprint_id") if User.current.allowed_to?(:view_agile_queries, nil, global: true)
          else
