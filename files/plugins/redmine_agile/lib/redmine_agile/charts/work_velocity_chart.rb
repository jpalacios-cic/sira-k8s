  ...
  module Charts
    class WorkVelocityChart < VelocityChart
      ...
      def calculate_velocity_data
        ...
        if @estimated_unit == 'hours'
          @created_by_period = issues_count_by_period(hours_created)
          @closed_by_period = [issues_count_by_period(hours_closed), issues_count_by_period(hours_partially_closed)].
              transpose.map{ |hour| hour.reduce(:+) }
        else
          sp_formatted_values = {}
          sp_partially_closed.each{|k, v| sp_formatted_values[k] = v.to_f}
          @created_by_period = issues_count_by_period(sp_created)
          @closed_by_period = [issues_count_by_period(sp_closed), issues_count_by_period(sp_formatted_values)].
      ...
      def hours_partially_closed
        @data_scope.
            ...
            group("#{Issue.table_name}.created_on").
            sum("(#{Issue.table_name}.estimated_hours * (#{Issue.table_name}.done_ratio) / 100.0)")
      end
      ...
      def sp_partially_closed
        @data_scope.
            ...
            joins(:agile_data).
            group("#{Issue.table_name}.created_on").
            sum("(#{AgileData.table_name}.story_points * (#{Issue.table_name}.done_ratio) / 100.0)")
      end
