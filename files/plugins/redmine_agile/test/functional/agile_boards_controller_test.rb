...
class AgileBoardsControllerTest < ActionController::TestCase
  ...
  def test_get_index_with_totals_when_without_time_entries_permission
    @request.session[:user_id] = 2
    ...
    assert_response :success
    assert_select 'input[value="spent_time"]'
    ...
    assert_response :success
    assert_select 'input[value="spent_time"]', 0
  end

  def test_get_index_with_scrum_board_and_backlog_column_and_empty_sprint
    @request.session[:user_id] = 1
    Project.find(1).enable_module!(:agile_backlog)
    last_sprint = AgileSprint.last
    sprint = AgileSprint.create!(
      project_id: 1,
      name: 'New test sprint',
      status: 0,
      start_date: (last_sprint.end_date + 1.day).strftime('%Y-%m-%d %H:%M:%S'),
      end_date: (last_sprint.end_date + 8.day).strftime('%Y-%m-%d %H:%M:%S')
    )
    params = {
      project_id: 'ecookbook',
      set_filter: '1',
      ...
      op: { 'status_id' => '=' },
      v: { 'status_id' => IssueStatus.ids },
      c: %w(tracker estimated_hours assigned_to)
    }

    compatible_request :get, :index, params
    assert_response :success
    assert_select 'td.issue-backlog-col'
  end

  def test_get_index_for_global_scrum_board_without_shared_sprints
    AgileSprint.update_all(sharing: AgileSprint.sharings[:none])
    compatible_request :get, :index, agile_query_params.merge(project_id: nil, sprints_enabled: '1')
    assert_response :success
    assert_select '.issue-card', 10
    assert_select 'select#sprint_id', 0
  end

  def test_get_index_for_global_scrum_board_if_used_sharing_sprint
    sprint = AgileSprint.first
    compatible_request :get, :index, agile_query_params.merge(project_id: nil, sprints_enabled: '1')
    assert_response :success
    assert_select '.issue-card', sprint.issues.count
  end

  def test_get_index_global_scrum_board_with_different_projects_if_used_sharing_sprint
    sprint = AgileSprint.first
    issue4 = Issue.find(4)
    EnabledModule.create(project: issue4.project, name: 'agile')
    issue4.agile_sprint = sprint
    compatible_request :get, :index, agile_query_params.merge(project_id: nil, sprints_enabled: '1')
    assert_response :success
    assert_select '.issue-card', sprint.issues.count
  end
