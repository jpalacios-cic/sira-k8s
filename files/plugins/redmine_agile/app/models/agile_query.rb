...
class AgileQuery < Query
  ...
  def sprint_id
    return @sprint_id if @sprint_id

    @sprint_id = options[:sprint_id] if options[:sprint_id]
    @sprint_id ||= project.active_sprint.try(:id) || project.shared_agile_sprints.available.first.try(:id) if project
    sharing_sprints = AgileSprint.available.system
    @sprint_id ||= sharing_sprints.active.first.try(:id) || sharing_sprints.open.first.try(:id)
  end
  ...
  def build_from_params(params)
    ...
    self.totalable_names = params[:t] || (params[:query] && params[:query][:totalable_names]) || totalable_names
  ...
  def agile_projects_condition
    ...
    return agile_pids.any? ? "#{Project.table_name}.id IN (#{agile_pids.join(',')})" : '1=1' if !project && sprints_enabled.to_i == 0
