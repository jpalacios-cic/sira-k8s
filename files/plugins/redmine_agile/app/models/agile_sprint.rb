...
class AgileSprint < ActiveRecord::Base
  ...
  scope :open_and_active, -> { where(status: [OPEN, ACTIVE]) }
  ...
  scope :shared, -> { where.not(sharing: AgileSprint.sharings[:none]) }
  ...
  def dates_crossing
    ...
    errors.add(:base, I18n.t(:label_agile_sprint_errors_crossed))
  end

  def issue_statuses
    errors.add(:base, I18n.t(:label_agile_sprint_errors_open_issues)) if status == CLOSED && issues.where(project_id: shared_projects.ids).open.any?
  end
